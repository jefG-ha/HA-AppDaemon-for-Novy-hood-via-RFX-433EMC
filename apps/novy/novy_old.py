...
class NovyHoodControl(hass.Hass):
    ...
    def initialize(self):
        ...
        self.hood_id        = bytes.fromhex(self.args["hood"].get("id", "000000"))
    ...
    def _mark_self_echo(self, cmd: int):
        # The RFXtrx echoes back the bursts of our own TX. Bursts complete
        # within ~150ms; 600ms gives a safe margin without suppressing a real
        # user press that follows shortly after our TX.
        with self.self_echo_lock:
            self.self_echoes.append((cmd, time.monotonic() + 0.6))
    ...
    def _handle_remote_press(self, cmd: int):
        ...
        # Filter out the burst of repeats that a single press generates.
        now = time.monotonic()
        with self.remote_lock:
            if cmd == self.last_remote_cmd and (now - self.last_remote_at) < REMOTE_DEDUP_WINDOW:
                # Refresh the timestamp so a long press (continuous repeats)
                # keeps being suppressed instead of triggering once per window.
    ...
    def _step_real(self, direction: int):
        new_real = self.real_speed + direction
        new_real = max(0, min(new_real, len(self.hood_speed_list) + 1))
        self.real_speed = new_real
        if new_real == 0:
            self.hood_state = "off"
        else:
            self.hood_state = "on"
            idx = max(0, min(new_real - 2, len(self.hood_speed_list) - 1))
